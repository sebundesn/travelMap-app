...
// handleListFriendPlaces returns every pin belonging to a confirmed friend.
func (s *server) handleListFriendPlaces(w http.ResponseWriter, r *http.Request) {
	userID := currentUserID(r)
	rows, err := s.db.Query(
		`SELECT p.id, p.name, p.country, p.lat, p.lng, p.visited_date, p.notes, p.image_url, p.created_at,
		        `+publicUserColumns+`
		 FROM places p
		 JOIN friendships f
	...
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to load friends' places")
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		var fp FriendPlace
		if err := rows.Scan(&fp.ID, &fp.Name, &fp.Country, &fp.Lat, &fp.Lng, &fp.VisitedDate,
			&fp.Notes, &fp.ImageURL, &fp.CreatedAt,
			&fp.Owner.ID, &fp.Owner.Name, &fp.Owner.Handle, &fp.Owner.AvatarURL, &fp.Owner.Bio); err != nil {
			writeError(w, http.StatusInternalServerError, "failed to read friends' places")
			return
		}
		places = append(places, fp)
	}
	if err := rows.Err(); err != nil {
		writeError(w, http.StatusInternalServerError, "failed to read friends' places")
		return
